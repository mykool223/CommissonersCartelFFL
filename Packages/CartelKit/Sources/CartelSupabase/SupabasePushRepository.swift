    ...
    public func setNotificationPreferences(_ preferences: NotificationPreferences) async throws {
        ...
        try await client.upsert(
            ...
            values: [
                ...
                "roster_news": AnyEncodable(preferences.rosterNews)
            ],
            onConflict: "user_id"
        )
    }
...
private struct PreferencesRow: Decodable {
    ...
    init(from decoder: any Decoder) throws {
        ...
        rosterNews = try container.decodeIfPresent(Bool.self, forKey: .rosterNews) ?? true
    }

    private enum CodingKeys: String, CodingKey {
        case messages, news, polls, activity, lineup, matchups, direct, mentions
        case rosterNews = "roster_news"
    }
}
